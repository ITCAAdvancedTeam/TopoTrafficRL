    ...
    def __eq__(self, other):
        if isinstance(other, Observation):
            ...
            for (x1, y1, vx1, vy1, a1), (x2, y2, vx2, vy2, a2) in zip(self.data, other.data):
                if not (abs(x1 - x2) < 0.5 and abs(y1 - y2) < 0.5 and
                        abs(vx1 - vx2) < 0.5 and abs(vy1 - vy2) < 0.5 and
                        abs(a1 - a2) < 0.1):
                    print("not eq")
                    return False
            print("eq")
    ...
    def sample(self, next_state, action, argmax=False):
        ...
        for i in range(k):
            ...
            if argmax:
                ...
                acceleration = norm.rvs(loc=next_state.data[i][2], scale=self.noise_level)
...
class IntersectionProblem(pomdp_py.POMDP):
    ...
    def __init__(self, obs_noise, init_true_state, init_belief, map):
        ...
        agent = pomdp_py.Agent(
            ...
            ObservationModel(map=map, noise_level=obs_noise),
            RewardModel(map=map),
        )
    ...
    def print_state(self):
        ...
        print("Terminal State:", "Yes" if state.terminate else "No")
...
def test_planner(intersection_problem, planner, nsteps=3, discount=0.95):
    ...
    for i in range(nsteps):
        ...
        print("action after plan: ", action)
        ...
        print("reward after transition: ", env_reward)
        ...
        if len(intersection_problem.agent.belief.particles) == 0:
            print("Warning: Particle deprivation detected. Check the transition and observation models.")
...
def main():
    # # Sample test code for the Observation class

    # # Define two observation data sets for testing
    # data1 = [(2.1536292412129328, -11.507487254852105, 4.947342536683504e-16, 8.079623512882305, -0.018012903828101064), (10.91088591688237, 1.4327184886799431, -7.932545032506466, 9.71456588315128e-16, -0.0814158520007995)]
    # data2 = [(1.9849056565334702, -10.86732909040776, 4.861968239365287e-16, 7.940196704470838, -0.16630095510766227), (11.598590060078287, 1.535925848461249, -8.042883585822008, 9.849691639291709e-16, 0.04103307934819003)]  # Slightly different from data1
    # data3 = [(1.8566478209110753, -11.349611621548144, 4.909846688194838e-16, 8.01838814524035, -0.006600721485248509), (11.348578428541297, 1.8823699521279904, -7.579505491836371, 9.282217139697197e-16, 0.002086303238585056)]  # Completely different

    # # Create Observation instances
    # obs1 = Observation(data1)
    # obs2 = Observation(data2)
    # obs3 = Observation(data3)

    # # Print the Observations
    # print("Observation 1:", obs1)
    # print("Observation 2:", obs2)
    # print("Observation 3:", obs3)

    # # Test equality
    # print("\nTesting Equality:")
    # print("obs1 == obs2:", obs1 == obs2)  # Should return True if differences are within tolerance
    # print("obs1 == obs3:", obs1 == obs3)  # Should return False

    # # Test hashability (can be used as keys in a dictionary)
    # print("\nTesting Hashability:")
    # obs_dict = {obs1: "First observation", obs3: "Third observation"}
    # print("Dictionary with Observation keys:", obs_dict)

    # # Test str and repr methods
    # print("\nTesting str and repr:")
    # print("str(obs1):", str(obs1))
    # print("repr(obs1):", repr(obs1))
    ...
    init_belief = init_particles_belief(num_particles=10000, init_state=init_true_state)

    problem = IntersectionProblem(obs_noise, init_true_state, init_belief, map)

    print("** Testing POMCP **")
    problem.agent.set_belief(
        pomdp_py.Particles.from_histogram(init_belief, num_particles=10000), prior=True
    )
